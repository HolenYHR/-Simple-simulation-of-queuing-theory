...
import numpy
class Packet:
    def __init__(self,arrivetime,sise):
    ...
    while True:
        ...
        for k in range(3):
           # print(len(A[k]),conTo[k])
            if  A[k][conTo[k]].arrivetime<A[min][conTo[min]].arrivetime:
                min=k
...
def PlotGraph2(array1,array2,array3):
    H1, X1 = numpy.histogram(array1, bins=70, normed=True)
    dx = X1[1] - X1[0]
    f1 = numpy.cumsum(H1) * dx
    # X2=numpy.sort(servicetime)
