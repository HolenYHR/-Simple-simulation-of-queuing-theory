...
number=0
def Simulation():
    global total,current,nextfinish,Upbound,Total,lamd,mu,conTo,conSys,min,number
    for i in range(3):
        ...
        if ts[min]!=Upbound:
            # print(type(ts[min]),type(nextfinish))
             if ts[min]<=nextfinish:
                 tt = tf[min]+rm.expovariate(lamd[min])
                 while tt<nextfinish:
                     scale=rm.expovariate(mu)
                     ...
                     conSys[min]+=1
                     tf[min]=tt
                     tt=tt+rm.expovariate(lamd[min])
                     total+=1
                 D[min].append(nextfinish)
                 number+=1
                # print("num:",number,"que:",min,"Arrive",A[min][conTo[min]].arrivetime,"virtime",A[min][conTo[min]].virtime,"size",A[min][conTo[min]].sise)
                 wa=nextfinish-A[min][conTo[min]].arrivetime
                # print(wa)
                 Wait[min].append(wa)
                 nextfinish=current+A[min][conTo[min]].sise
                 conSys[min]-=1
